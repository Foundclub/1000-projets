...
/**
 * POST /api/missions/[id]/applications/[applicationId]/reject
 * Rejette une candidature pour une mission
 */
export async function POST(
  ...
  try {
    ...
    try {
      await createNotification(application.userId, 'APPLICATION_REJECTED', {
        missionId: mission.id,
        missionTitle: mission.title,
        applicationId: application.id,
      });
    } catch (notifError) {
      console.error('Error creating notification for application rejection:', notifError);
    }

    return NextResponse.json({ application: updated });
  } catch (e: any) {
    console.error('Error in POST /api/missions/[id]/applications/[applicationId]/reject:', e);
